...
struct bdm_EXT_mdblock 
{
    ...
    u32 startSector;                  /* Local block address to first sector of partition */
    const bool stopSectors;
    struct block_device **ntfspartitions;  
};
struct BDM_DISC_INTERFACE;
 ...
 */
int bdmext2FindPartitions(const BDM_DISC_INTERFACE *interface, sec_t **partitions);
 ...
 */
bool bdmext2Mount(const char *name, const BDM_DISC_INTERFACE *interface, sec_t startSector, u32 cachePageCount, u32 cachePageSize, u32 flags);
 ...
 */
void bdmext2Unmount(const char *name);
 ...
 */
bool bdmext2SetVolumeName (const char *name, const char *volumeName);
